...
function InvoiceModal({ cases, onClose, isBulk, existingInvoice }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/invoices/save", {
        method: "POST",
        headers: {
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <select 
                        ...
                        onChange={e => {
                          ...
                          fetch(`http://localhost:5000/api/invoices?bank=${bank === "All" ? "" : bank}&month=${month === "All" ? "" : month}&year=${year === "All" ? "" : year}&status=${status === "All" ? "" : status}`, {
                            headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` },
                            credentials: "include"
                          })
                          .then(r => r.json())
